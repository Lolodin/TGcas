...
func ConnectBinary(signal, stopsignal, static, testSignal chan int, bot *tgbotapi.BotAPI, stor *store.MySQL) {
	...
	func() {
		...
		r18 := []byte(`{"ticks_history":"1HZ10V","style":"ticks","end":"latest","count":20,"subscribe":1,"req_id":14,"passthrough":{}}`)
		r19 := []byte(`{"proposal":1,"subscribe":1,"amount":10,"basis":"stake","contract_type":"CALL","currency":"USD","symbol":"1HZ10V","duration":5,"duration_unit":"t","passthrough":{"form_id":1},"req_id":15}`)
		r20 := []byte(`{"proposal":1,"subscribe":1,"amount":10,"basis":"stake","contract_type":"PUT","currency":"USD","symbol":"1HZ10V","duration":5,"duration_unit":"t","passthrough":{"form_id":1},"req_id":16}`)
